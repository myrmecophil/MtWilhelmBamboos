...
#
bamboo.diversity.model1 <- glmmTMB(expH ~ Forest+new.Treatment+Stratum+(1|Block/Plot), 
                                   zi=~Forest, 
                                   data = diversity.nester.e, 
                                   family=gaussian)
bamboo.diversity.model2 <- glmmTMB(expH ~ Forest*Stratum+new.Treatment+(1|Block/Plot), 
                                   zi=~Forest*Stratum,
                                   data = diversity.nester.e, 
                                   family=gaussian)
...
testZeroInflation(simulateResiduals(fittedModel = bamboo.diversity.model1)) # even with zi factor zero inflated (p=0.008)
...
anova(bamboo.diversity.model1, bamboo.diversity.model.e1)
...
summary(baitoccupancy.model1)
baitoccupancy.model1 %>% report()

summary(baitoccupancy.model.e1)

# - higher elevation baits -> less occupancy
# - understory baits have slitghly lower occupancy
...
# - slight positive influence of slope on abundance

# 4) Does bait diversity change?
summary(baitdiversity.stratum.model1)
baitdiversity.stratum.model1 %>% report()
...
summary(bamboo.occupancy.model2)
bamboo.occupancy.model2 %>% report()
...
summary(bamboo.abundance.model4)
bamboo.abundance.model4 %>% report()
...
summary(bamboo.diversity.model1)
bamboo.diversity.model1 %>% report()
...
### --- CODE NOT WORKING -- maybe try to make it work some other time 
