...
tree.meta$trunk<-log(tree.meta$trunk+1)
#
hist(tree.meta$Lianas.n)#right skewed, log+1 transformation fine
hist(log(tree.meta$Lianas.n+1))
tree.meta$Lianas.n<-log(tree.meta$Lianas.n+1)
#
hist(tree.meta$height)#right skewed, maybe sqrt transformation?
hist(sqrt(tree.meta$height))
tree.meta$height<-sqrt(tree.meta$height)
#  
hist(tree.meta$Caco)#looks ok
#
hist(tree.meta$dw.percent)#right skewed, log+1 transformation fine?
hist(log(tree.meta$dw.percent+1))
tree.meta$dw.percent<-log(tree.meta$dw.percent+1)
#
hist(tree.meta$dw.number)#right skewed, log+1 transformation fine?
hist(log(tree.meta$dw.number+1))
tree.meta$dw.number<-log(tree.meta$dw.number+1) 
#
hist(tree.meta$slope.var)#slightly right skewed, sqrt transformation fine
hist(sqrt(tree.meta$slope.var))
tree.meta$slope.var<-log(tree.meta$slope.var+1) 
...
##### Statistic:  Are any plot attributes different between elevations?
...
model_dbh<- lmer(trunk ~  elevation + (1|Block)+(1|Plot), data = tree.meta)
summary(model_dbh)

# Is number of deadwood pieces different?
model_dw.n<- lmer(dw.number ~  elevation + (1|Block)+(1|Plot), data= tree.meta)
summary(model_dw.n)

# is % deadwood cover different?
model_dw.p<- lmer(dw.percent ~  elevation + (1|Block)+(1|Plot), data= tree.meta)
summary(model_dw.p)

# Is number of lianas different?
model_lianas<- lmer(Lianas.n ~  elevation + (1|Block)+(1|Plot), data= tree.meta)
summary(model_lianas)
# => slighly more lianas in higher elevation

# bamboo/bait height different?
model_height<- lmer(height ~  elevation + (1|Block)+(1|Plot), data= tree.meta)
...
baitoccupancy.model <- glmmTMB(occupancy~elevation+(1|Block)+(1|Plot)+(1|tree.ID),
                               data=baiting.incidence.e,
                               family=binomial)
summary(baitoccupancy.model)

# with environmental factors
baitoccupancy.model.e <- glmmTMB(occupancy~elevation+Lianas.n+height+dw.percent+dw.number+trunk+Caco+slope.var+(1|Block)+(1|Plot),
                               data=baiting.incidence.e,
                               family=binomial)
summary(baitoccupancy.model.e)

# with abundance 
baitoccupancy.model1 <- glmmTMB(Abundance~elevation+(1|Block)+(1|Plot),
                               data=baiting.incidence.e,
                               family=nbinom1)
summary(baitoccupancy.model1)

# with abundance and environmental
baitoccupancy.model.e1 <- glmmTMB(Abundance~elevation+Lianas.n+height+dw.percent+dw.number+trunk+Caco+slope.var+(1|Block)+(1|Plot),
                                 data=baiting.incidence.e,
                                 family=nbinom1)
...
baitdiversity.model <- lmer(evenness ~ elevation + (1|Block), data = baiting.diversity.e)
summary(baitdiversity.model)

# add environmental factors
baitdiversity.model.e <- lmer(evenness ~elevation+slope.var+Caco+trunk_mean+Lianas.n_mean+dw.number_mean+dw.percent_mean + (1|Block),
                              data = baiting.diversity.e)
summary(baitdiversity.model.e)
...
bamboo.occupancy.model <- glmmTMB(occupancy~elevation+phase+(1|Block)+(1|Plot),
                                  data=bamboo.incidence.e,
                                  family=binomial)
summary(bamboo.occupancy.model)

# binominal model of bamboo nesting using only controls
bamboo.occupancy.model <- glmmTMB(occupancy~elevation+phase+(1|Plot),
...
bamboo.occupancy.model2 <- glmmTMB(occupancy~elevation+new.Treatment+(1|Block)+(1|Plot),
                                   data=bamboo.incidence.e,
                                   family=binomial)
...
bamboooccupancy.model.e <- glmmTMB(occupancy~elevation+Caco+slope.var+Lianas.n+height+dw.percent+ dw.number+ trunk+(1|Block)+(1|Plot),
                                 data=bamboo.incidence.e,
                                 family=binomial)
summary(bamboooccupancy.model.e)

# with abundance 
bamboo.occupancy.model3 <- glmmTMB(nesting.estimate~elevation+phase+(1|Block)+(1|Plot),
                                data=bamboo.incidence.e,
                                family=nbinom2)
summary(bamboo.occupancy.model3)

# with abundance and environmental
bamboo.occupancy.model.e4 <- glmmTMB(nesting.estimate~elevation+slope.var+Lianas.n+height+dw.percent+ dw.number+ trunk+Caco+phase+(1|Block)+(1|Plot),
                                  data=bamboo.incidence.e,
                                  family=nbinom1)
...
# diversity changes with location & phase?
...
bamboo.diversity.model <- lmer(expH ~ elevation+slope.var+Caco+trunk_mean+Lianas.n_mean+dw.number_mean+dw.percent_mean+phase+ (1|Block), data = diversity.nester.e)
summary(bamboo.diversity.model)
